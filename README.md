...
### ALL THE GAMES SHOWN AND TESTED ARE FROM FULLY LEGAL COPIES I OWN AND WERE PERSONALLY DUMPED FROM EITHER PHYSICAL CARTRIDGES OR A REAL NINTENDO 3DS RUNNING VIRTUAL CONSOLE SOFTWARE. 
...
<img src= "https://github.com/user-attachments/assets/30b5cf56-463f-422c-93ba-8e5fe993504b" width="250">
...
## Designing the Software Structure.
  ...
  * One major difference between my emulator and a lot of others was the focus on using one large uint8_t SystemMemory[0x10000] array. The primary reason for this was to allow for easier implementation of save-state support later on, and also because it felt more true to the actual system as far as I read documentation-wise in terms of the way addressing modes and the Gameboy memory map worked. For obvious reasons, this is not an optimal choice for performance, but it makes the architecture feel more true to hardware, as it would be easier to conver to Verilog FPGA code. I made a lot of design decisions similar to this, including the big if-else tree of the CPU for this reason. In that specific case a function pointer tree would be more optimal, but perhaps a bit dangerous to code, and impossible to convert to actual hardware. 
